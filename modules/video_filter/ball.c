                           ...
                           picture_t *p_outpic )
{
    ...
    for ( i_motion = 0; i_motion <= p_sys->i_ballSpeed && !bounce; i_motion++ )
    {
        ...
        x = cover_roundf( (float)p_sys->i_ball_x
                    + *pf_lastVect_x * (float)i_motion );			// sunqueen modify
        y = cover_roundf( (float)p_sys->i_ball_y
                    + *pf_lastVect_y * (float)i_motion );			// sunqueen modify
    ...
    if( bounce )
    {
        ...
        *pf_lastVect_y = f_newVect_y;
        ...
        x = cover_roundf( (float)x + *pf_lastVect_x );			// sunqueen modify
        y = cover_roundf( (float)y + *pf_lastVect_y );			// sunqueen modify
    ...
    /* Compute next ball position. */
    p_sys->i_ball_x = cover_roundf( (float)p_sys->i_ball_x + *pf_lastVect_x
                              * (float)( p_sys->i_ballSpeed - i_motion ) );			// sunqueen modify
    p_sys->i_ball_y = cover_roundf( p_sys->i_ball_y + *pf_lastVect_y
                              * (float)( p_sys->i_ballSpeed - i_motion ) );			// sunqueen modify
