...
static int AddStream( sout_mux_t *p_mux, sout_input_t *p_input )
{
    ...
    p_stream                = (mp4_stream_t *)malloc( sizeof( mp4_stream_t ) );			// sunqueen modify
    if( !p_stream )
        return VLC_ENOMEM;
    ...
    p_stream->entry         = (mp4_entry_t *)
        calloc( p_stream->i_entry_max, sizeof( mp4_entry_t ) );			// sunqueen modify
...
again:
        ...
        {
            p_stream->i_entry_max += 1000;
            p_stream->entry = (mp4_entry_t *)xrealloc( p_stream->entry,
                         p_stream->i_entry_max * sizeof( mp4_entry_t ) );			// sunqueen modify
        }
...
static bo_t *GetAvcCTag( mp4_stream_t *p_stream )
{
    ...
    if( p_stream->fmt.i_extra > 0 )
    {
        ...
        uint8_t *p_buffer = (uint8_t *)p_stream->fmt.p_extra;			// sunqueen modify
                     ...
                     bool b_grow )
{
    if( !p_buffer )
    {
        ...
        p_bo->p_buffer = (uint8_t *)xmalloc( p_bo->i_buffer_size );			// sunqueen modify
    }
    else
    {
...
static void bo_add_8( bo_t *p_bo, uint8_t i )
{
    ...
    else if( p_bo->b_grow )
    {
        ...
        p_bo->p_buffer = (uint8_t *)xrealloc( p_bo->p_buffer, p_bo->i_buffer_size );			// sunqueen modify
        p_bo->p_buffer[p_bo->i_buffer] = i;
    }
...
static bo_t * box_new( const char *fcc )
{
    ...
    if( ( box = (bo_t *)malloc( sizeof( bo_t ) ) ) )			// sunqueen modify
    {
