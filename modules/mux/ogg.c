...
static int AddStream( sout_mux_t *p_mux, sout_input_t *p_input )
{
    ...
    case AUDIO_ES:
        ...
        default:
            ...
            p_stream->p_oggds_header = (oggds_header_t *)
                malloc( sizeof(oggds_header_t) + p_input->p_fmt->i_extra );			// sunqueen modify
            if( !p_stream->p_oggds_header )
            {
                free( p_stream );
                return VLC_ENOMEM;
            ...
            char buf[5];
            _snprintf( buf, sizeof(buf), "%"PRIx16, i_tag );			// sunqueen modify
...
static int DelStream( sout_mux_t *p_mux, sout_input_t *p_input )
{
    ...
    if( p_input->p_sys )
    {
        ...
        if( !p_stream->b_new )
        {
            p_sys->pp_del_streams = (ogg_stream_t **)xrealloc( p_sys->pp_del_streams,
                        (p_sys->i_del_streams + 1) * sizeof(ogg_stream_t *) );			// sunqueen modify
...
static block_t *OggCreateHeader( sout_mux_t *p_mux )
{
    ...
    for( i = 0; i < p_mux->i_nb_inputs; i++ )
    {
                 ...
                 p_stream->i_fourcc != VLC_CODEC_DIRAC )
        {
            ...
            i_com = _snprintf( (char *)(com+1), 127,
                              PACKAGE_VERSION" stream output" )			// sunqueen modify
                     + 1;
            op.packet = com;
            op.bytes  = i_com;
            op.b_o_s  = 0;
