...
//    tk = p_input->p_sys = malloc( sizeof( asf_track_t ) );
    p_input->p_sys = malloc( sizeof( asf_track_t ) );
    ...
    switch( tk->i_cat )
    {
        case AUDIO_ES:
        {
            ...
            tk->p_extra = (uint8_t *)malloc( tk->i_extra );			// sunqueen modify
            if( !tk->p_extra )
                return VLC_ENOMEM;
            ...
            if( p_input->p_fmt->i_extra > 0 )
            {
                ...
                bo_add_mem  ( &bo, (uint8_t *)p_input->p_fmt->p_extra,
                              p_input->p_fmt->i_extra );			// sunqueen modify
            }
            else
            {
        ...
        case VIDEO_ES:
        {
            ...
            else if( p_input->p_fmt->i_codec == VLC_CODEC_VC1 )
            {
                ...
                if( p_fmt->i_extra > 0 )
                {
                    p_codec_extra = (uint8_t *)malloc( 1 + p_fmt->i_extra );			// sunqueen modify
                    if( p_codec_extra )
                    {
            ...
            {
                p_codec_extra = (uint8_t *)malloc( p_fmt->i_extra );			// sunqueen modify
                if( p_codec_extra )
                {
...
static int DelStream( sout_mux_t *p_mux, sout_input_t *p_input )
{
     ...
     */
    sout_mux_sys_t   *p_sys = p_mux->p_sys;
    asf_track_t      *tk = (asf_track_t *)p_input->p_sys;			// sunqueen modify
...
static block_t *asf_header_create( sout_mux_t *p_mux, bool b_broadcast )
{
    ...
    {
        tk = (asf_track_t *)vlc_array_item_at_index( p_sys->p_tracks, i );			// sunqueen modify
    ...
    for( i = 0; i < vlc_array_count( p_sys->p_tracks ); i++ )
    {
        const asf_track_t *p_track = (asf_track_t *)vlc_array_item_at_index( p_sys->p_tracks, i );			// sunqueen modify
