...
static int Open( vlc_object_t *p_this )
{
    ...
    p_sys->idx1.entry = (avi_idx1_entry_t *)calloc( p_sys->idx1.i_entry_max,
                                sizeof( avi_idx1_entry_t ) );			// sunqueen modify
    if( !p_sys->idx1.entry )
    {
        free( p_sys );
        return VLC_ENOMEM;
    ...
    *((int*)p_input->p_sys) = p_sys->i_streams;
    ...
    switch( p_input->p_fmt->i_cat )
    {
        case AUDIO_ES:
            ...
            p_stream->p_wf  = (WAVEFORMATEX *)malloc( sizeof( WAVEFORMATEX ) +
...
#undef p_wf
        ...
        case VIDEO_ES:
            ...
            p_stream->p_wf  = NULL;
            p_stream->p_bih = (VLC_BITMAPINFOHEADER *)malloc( sizeof( VLC_BITMAPINFOHEADER ) +
                                      p_input->p_fmt->i_extra );			// sunqueen modify
            if( !p_stream->p_bih )
            {
                free( p_input->p_sys );
...
static int Mux      ( sout_mux_t *p_mux )
{
    ...
    for( i = 0; i < p_mux->i_nb_inputs; i++ )
    {
        ...
        while( i_count > 1 )
        {
            ...
            {
                p_sys->idx1.i_entry_max += 10000;
                p_sys->idx1.entry = (avi_idx1_entry_t *)xrealloc( p_sys->idx1.entry,
                       p_sys->idx1.i_entry_max * sizeof( avi_idx1_entry_t ) );			// sunqueen modify
            }
