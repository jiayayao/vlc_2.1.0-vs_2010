...
#define ALLOC_ITEM_META(a, b) do { \
    ...
    if (!p_sys->p_current_song.psz_b)
        p_sys->p_current_song.psz_b = (char *)calloc(1, 1);			// sunqueen modify

    ALLOC_ITEM_META(p_sys->p_current_song.psz_m, TrackID);
    if (!p_sys->p_current_song.psz_m)
        p_sys->p_current_song.psz_m = (char *)calloc(1, 1);			// sunqueen modify
    ...
    if (!p_sys->p_current_song.psz_n)
        p_sys->p_current_song.psz_n = (char *)calloc(1, 1);			// sunqueen modify
...
static int Open(vlc_object_t *p_this)
{
    ...
    intf_sys_t      *p_sys      = (intf_sys_t *)calloc(1, sizeof(intf_sys_t));			// sunqueen modify
...
static int Handshake(intf_thread_t *p_this)
{
    ...
    _snprintf(psz_timestamp, sizeof(psz_timestamp), "%"PRIu64,
              (uint64_t)timestamp);			// sunqueen modify
