...
static int OpenOut( vlc_object_t *p_this )
{
    ...
    p_sys          = (out_sout_stream_sys_t *)malloc( sizeof( out_sout_stream_sys_t ) );			// sunqueen modify
    if( unlikely( !p_sys ) )
        return VLC_ENOMEM;
...
static sout_stream_id_t * AddOut( sout_stream_t *p_stream, es_format_t *p_fmt )
{
    ...
    p_bridge = (bridge_t *)var_GetAddress( p_stream->p_libvlc, p_sys->psz_name );			// sunqueen modify
    if ( p_bridge == NULL )
    {
    ...
    {
        p_bridge->pp_es = (bridged_es_t **)xrealloc( p_bridge->pp_es,
                          (p_bridge->i_es_num + 1) * sizeof(bridged_es_t *) );			// sunqueen modify
        p_bridge->i_es_num++;
...
static int OpenIn( vlc_object_t *p_this )
{
    ...
    p_sys          = (in_sout_stream_sys_t *)malloc( sizeof( in_sout_stream_sys_t ) );			// sunqueen modify
    if( unlikely( !p_sys ) )
        return VLC_ENOMEM;
...
static sout_stream_id_t * AddIn( sout_stream_t *p_stream, es_format_t *p_fmt )
{
    ...
    sout_stream_id_t *id = (sout_stream_id_t *)malloc( sizeof( sout_stream_id_t ) );			// sunqueen modify
    if( !id ) return NULL;
