...
static int Del( sout_stream_t *p_stream, sout_stream_id_t *id )
{
    ...
    for( i_stream = 0; i_stream < p_sys->i_nb_streams; i_stream++ )
    {
        ...
        {
            sout_stream_t *out = p_sys->pp_streams[i_stream];
            sout_StreamIdDel( out, (sout_stream_id_t *)id->pp_ids[i_stream] );			// sunqueen modify
        }
    }
    ...
    while( p_buffer )
    {
        ...
        for( i_stream = 0; i_stream < p_sys->i_nb_streams - 1; i_stream++ )
        {
            ...
            if( id->pp_ids[i_stream] )
            {
                ...
                if( p_dup )
                    sout_StreamIdSend( p_dup_stream, (sout_stream_id_t *)id->pp_ids[i_stream], p_dup );			// sunqueen modify
            }
        }
        ...
        {
            p_dup_stream = p_sys->pp_streams[i_stream];
            sout_StreamIdSend( p_dup_stream, (sout_stream_id_t *)id->pp_ids[i_stream], p_buffer );			// sunqueen modify
        }
        else
        {
