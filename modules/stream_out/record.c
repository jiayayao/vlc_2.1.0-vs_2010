...
static int Open( vlc_object_t *p_this )
{
    ...
    p_stream->p_sys = p_sys = (sout_stream_sys_t *)malloc( sizeof(*p_sys) );			// sunqueen modify
    if( !p_sys )
        return VLC_ENOMEM;
...
static sout_stream_id_t *Add( sout_stream_t *p_stream, es_format_t *p_fmt )
{
    ...
    id = (sout_stream_id_t *)malloc( sizeof(*id) );			// sunqueen modify
    if( !id )
        return NULL;
...
#define M(muxer, ext, count, ... ) { /*.psz_muxer = */ muxer, /*.psz_extension = */ ext, /*.i_es_max = */ count, /*.codec = */ { __VA_ARGS__, 0 } }			// sunqueen modify
