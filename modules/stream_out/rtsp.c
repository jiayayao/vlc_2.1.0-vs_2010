...
static void RtspTimeOut( void *data )
{
    ...
    for (int i = rtsp->sessionc - 1; i >= 0; i--)
    {
        if (rtsp->sessionv[i]->last_seen + rtsp->timeout * CLOCK_FREQ < now)
        {
            ...
            {
                char psz_sesbuf[17];
                _snprintf( psz_sesbuf, sizeof( psz_sesbuf ), "%"PRIx64,
                          rtsp->sessionv[i]->id );			// sunqueen modify
                 ...
                 tpt = transport_next( tpt ) )
            {
                ...
                if( b_multicast )
                {
                    ...
                    if( psz_session == NULL )
                    {
                        ...
                        _snprintf( psz_sesbuf, sizeof( psz_sesbuf ), "%lu",
                                  vlc_mrand48() );			// sunqueen modify
...
//                                sizeof (int));
                    ...
                    if (tr == NULL)
                    {
                        ...
                        INSERT_ELEM( (rtsp_strack_t *), ses->trackv, ses->trackc, ses->trackc,
                                     track );			// sunqueen modify
                    }
