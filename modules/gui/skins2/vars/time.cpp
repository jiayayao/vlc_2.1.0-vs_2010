...
string StreamTime::getAsStringPercent() const
{
    ...
    char str[4];
    _snprintf( str, 4, "%d", value );			// sunqueen modify
    return string(str);
}
...
string StreamTime::formatTime( int seconds, bool bShortFormat ) const
{
    ...
    {
        _snprintf( psz_time, MSTRTIME_MAX_SIZE, "%02d:%02d",
                  (int) (seconds / 60 % 60),
                  (int) (seconds % 60) );			// sunqueen modify
    }
    else
    {
        _snprintf( psz_time, MSTRTIME_MAX_SIZE, "%d:%02d:%02d",
                  (int) (seconds / (60 * 60)),
                  (int) (seconds / 60 % 60),
                  (int) (seconds % 60) );			// sunqueen modify
    }
