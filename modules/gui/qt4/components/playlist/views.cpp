...
{
    QString title = VLCModel::getMeta( index, COLUMN_TITLE__ );			// sunqueen modify
    QString artist = VLCModel::getMeta( index, COLUMN_ARTIST__ );			// sunqueen modify
...
{
    QString title = VLCModel::getMeta( index, COLUMN_TITLE__ );			// sunqueen modify
    QString duration = VLCModel::getMeta( index, COLUMN_DURATION__ );			// sunqueen modify
    if( !duration.isEmpty() ) title += QString(" [%1]").arg( duration );

    QString artist = VLCModel::getMeta( index, COLUMN_ARTIST__ );			// sunqueen modify
    QString album = VLCModel::getMeta( index, COLUMN_ALBUM__ );			// sunqueen modify
    QString trackNum = VLCModel::getMeta( index, COLUMN_TRACK_NUMBER__ );			// sunqueen modify
