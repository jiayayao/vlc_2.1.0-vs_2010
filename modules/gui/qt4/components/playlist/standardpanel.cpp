 ...
 * $Id: 0a2b3f29ccb1b5f427e901ba8d3815ac2b53d452 $
 *
...
#define ADD_MENU_ENTRY( icon, title, act, data ) \
    ...
    sortingColumns << COLUMN_TITLE__ << COLUMN_ARTIST__ << COLUMN_ALBUM__ << COLUMN_TRACK_NUMBER__ << COLUMN_URI__;			// sunqueen modify
...
void StandardPLPanel::popupSelectColumn( QPoint )
{
    ...
    for( int i = 1 << 1, j = 1; i < COLUMN_END__; i <<= 1, j++ )			// sunqueen modify
    {
...
void StandardPLPanel::showView( int i_view )
{
    ...
    if( b_treeViewCreated )
    {
        ...
        {
            for( int m = 1, c = 0; m != COLUMN_END__; m <<= 1, c++ )			// sunqueen modify
            {
                treeView->setColumnHidden( c, !( m & COLUMN_DEFAULT__ ) );			// sunqueen modify
                if( m == COLUMN_TITLE__ ) treeView->header()->resizeSection( c, 200 );			// sunqueen modify
                else if( m == COLUMN_DURATION__ ) treeView->header()->resizeSection( c, 80 );			// sunqueen modify
            }
        }
    }
