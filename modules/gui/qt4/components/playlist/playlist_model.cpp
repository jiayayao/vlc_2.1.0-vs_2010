 ...
 * $Id: 932a4b07788e9876e3cfdaaeb76992b923063963 $
 *
...
QVariant PLModel::data( const QModelIndex &index, /*const*/ int role ) const			// sunqueen modify
{
    ...
    if( role == Qt::DisplayRole )
    {
        ...
        if( metadata == COLUMN_END__ ) return QVariant();			// sunqueen modify

        QString returninfo;
        if( metadata == COLUMN_NUMBER__ )			// sunqueen modify
            returninfo = QString::number( index.row() + 1 );
        else if( metadata == COLUMN_COVER__ )			// sunqueen modify
        {
            QString artUrl;
    ...
    else if( role == Qt::ToolTipRole )
    {
        ...
        {
            char *psz = psz_column_meta( item->inputItem(), COLUMN_DURATION__ );			// sunqueen modify
            duration = qfu( psz );
            free( psz );
        }
                              ...
                              int role ) const
{
    ...
    if( meta_col == COLUMN_END__ ) return QVariant();			// sunqueen modify

    return QVariant( qfu( psz_column_title( meta_col ) ) );
}

QModelIndex PLModel::index( /*const*/ int row, /*const*/ int column, const QModelIndex &parent )			// sunqueen modify
                  const
{
...
QModelIndex PLModel::index( /*const*/ int i_id, /*const*/ int c )			// sunqueen modify
{
...
void PLModel::sort( QModelIndex caller, QModelIndex rootIndex, const int column, Qt::SortOrder order )
{
    ...
    if( meta == COLUMN_END__ ) return;			// sunqueen modify
