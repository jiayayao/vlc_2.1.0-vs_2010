...
static int Open( vlc_object_t *p_this )
{
    ...
    if( !( p_sys = p_access->p_sys = (sout_access_out_sys_t *)
                malloc( sizeof( sout_access_out_sys_t ) ) ) )			// sunqueen modify
        return VLC_ENOMEM ;
...
//            char host[len + 1];
    ...
    p_sys->p_header           = (uint8_t *)xmalloc( p_sys->i_header_allocated );			// sunqueen modify
        ...
        if( p_buffer->i_flags & BLOCK_FLAG_HEADER )
        {
                ...
                p_sys->i_header_allocated )
            {
                ...
                p_sys->p_header = (uint8_t *)xrealloc( p_sys->p_header,
                                                  p_sys->i_header_allocated );			// sunqueen modify
            }
