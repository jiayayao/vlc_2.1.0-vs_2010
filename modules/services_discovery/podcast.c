...
static int Open( vlc_object_t *p_this )
{
    ...
    services_discovery_sys_t *p_sys  = (services_discovery_sys_t *)malloc(
                                    sizeof( services_discovery_sys_t ) );			// sunqueen modify
    if( !p_sys )
        return VLC_ENOMEM;
...
static void ParseUrls( services_discovery_t *p_sd, char *psz_urls )
{
    ...
    for( ;; )
    {
        ...
        if( i == p_sys->i_urls )
        {
            INSERT_ELEM( (char **), ppsz_new_urls, i_new_urls, i_new_urls,
                         strdup( psz_urls ) );			// sunqueen modify
            ...
            INSERT_ELEM( (input_thread_t **), p_sys->pp_input, p_sys->i_input, p_sys->i_input,
                         input_CreateAndStart( p_sd, p_input, NULL ) );			// sunqueen modify
        }
        else
        {
            INSERT_ELEM( (char **), ppsz_new_urls, i_new_urls, i_new_urls,
                         strdup( p_sys->ppsz_urls[i]) );			// sunqueen modify
...
static void ParseRequest( services_discovery_t *p_sd )
{
    ...
    *psz_tok = '\0';
    ...
    if( !strcmp( psz_request, "ADD" ) )
    {
        ...
        if( i == p_sys->i_urls )
        {
            INSERT_ELEM( (char **), p_sys->ppsz_urls, p_sys->i_urls, p_sys->i_urls,
              strdup( psz_request ) );			// sunqueen modify
