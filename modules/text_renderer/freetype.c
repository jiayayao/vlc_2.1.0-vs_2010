...
static int GetFileFontByName( LPCTSTR font_name, char **psz_filename )
{
    ...
    char *font_name_temp = FromT( (const char *)font_name );			// sunqueen modify
    ...
    for( int index = 0;; index++ )
    {
        ...
        char *psz_value = FromT( (const char *)vbuffer );			// sunqueen modify
    ...
    *psz_filename = FromT( (const char *)dbuffer );			// sunqueen modify
    free( font_name_temp );
    RegCloseKey( hKey );
    return 0;
}
...
static FT_Face LoadEmbeddedFace( filter_sys_t *p_sys, const text_style_t *p_style )
{
    for( int k = 0; k < p_sys->i_font_attachments; k++ )
    {
        ...
        while( 0 == FT_New_Memory_Face( p_sys->p_library,
                                        (const FT_Byte *)p_attach->p_data,
                                        p_attach->i_data,
                                        i_font_idx,
                                        &p_face ))			// sunqueen modify
        {
            if( p_face )
            {
