...
static block_t *DoWork( filter_t *p_filter, block_t *p_in_buf )
{
    ...
    if (p_sys->silence) {
        ...
        if ((p_sys->started) && (p_in_buf->i_pts > p_sys->lastAlarm + (p_sys->repetition_time*1000))) {
            ...
            _snprintf(message,255,"@audiobargraph_v audiobargraph_v-alarm %d\n",i);			// sunqueen modify
    ...
    if (p_sys->bargraph) {
        ...
        if ((nbChannels > 0) && (p_sys->counter%(p_sys->bargraph_repetition) == 0)) {
            j=_snprintf(message,255,"@audiobargraph_v audiobargraph_v-i_values ");			// sunqueen modify
            for (i=0; i<(nbChannels-1); i++) {
                j+=_snprintf(message+j,255,"%f:", i_value[i]);			// sunqueen modify
            }
            _snprintf(message+j,255,"%f\n", i_value[nbChannels-1]);			// sunqueen modify
