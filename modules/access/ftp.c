...
static int ftp_RecvAnswer( vlc_object_t *obj, access_sys_t *sys,
                           int *__restrict codep, char **__restrict strp,
                           void (*cb)(void *, const char *), void *opaque )			// sunqueen modify
{
    if( codep != NULL )
        *codep = 500;
    if( strp != NULL )
...
static int ftp_RecvCommand( vlc_object_t *obj, access_sys_t *sys,
                            int *__restrict codep, char **__restrict strp )			// sunqueen modify
{
...
{
    bool *unicode = (bool *)opaque;			// sunqueen modify
...
static int OutOpen( vlc_object_t *p_this )
{
    ...
    p_sys = (access_sys_t *)calloc( 1, sizeof( *p_sys ) );			// sunqueen modify
    if( !p_sys )
        return VLC_ENOMEM;
    ...
    p_access->p_sys = (sout_access_out_sys_t *)p_sys;			// sunqueen modify
...
static ssize_t Read( access_t *p_access, uint8_t *p_buffer, size_t i_len )
{
    ...
    if( p_sys->directory )
    {
        ...
        {
            _snprintf( (char*)p_buffer, i_len, "ftp://%s:%d/%s/%s\n",
                      p_sys->url.psz_host, p_sys->url.i_port,
                      p_sys->url.psz_path, psz_line );			// sunqueen modify
            free( psz_line );
