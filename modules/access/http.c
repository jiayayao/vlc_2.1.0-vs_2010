...
static int ReadICYMeta( access_t *p_access )
{
    ...
    psz_meta = (char *)malloc( i_size + 1 );			// sunqueen modify
                               ...
                               size_t i_len )
{
    ...
    if( p_sys->b_compressed )
    {
        ...
        if( !p_sys->inflate.p_buffer )
            p_sys->inflate.p_buffer = (uint8_t *)malloc( 256 * 1024 );			// sunqueen modify
...
//    setsockopt (p_sys->fd, SOL_SOCKET, SO_KEEPALIVE, &(int){ 1 }, sizeof (int));
	int opt = 1;
    setsockopt (p_sys->fd, SOL_SOCKET, SO_KEEPALIVE, (const char *)&opt, sizeof (int));
    // sunqueen modify end
...
static int Request( access_t *p_access, uint64_t i_tell )
{
    ...
    if( p_sys->cookies )
    {
        ...
        for( i = 0; i < vlc_array_count( p_sys->cookies ); i++ )
        {
            const char * cookie = (const char *)vlc_array_item_at_index( p_sys->cookies, i );			// sunqueen modify
...
static void cookie_append( vlc_array_t * cookies, char * cookie )
{
    ...
    {
        char * current_cookie = (char *)vlc_array_item_at_index( cookies, i );			// sunqueen modify
