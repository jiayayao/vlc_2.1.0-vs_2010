...
int var_buffer_initwrite( var_buffer_t *p_buf, int i_default_size )
{
    ...
    p_buf->i_data = 0;
    p_buf->p_data = (uint8_t *)malloc( p_buf->i_size );			// sunqueen modify
...
void var_buffer_add8 ( var_buffer_t *p_buf, uint8_t  i_byte )
{
    ...
    {
        p_buf->i_size += 1024;
        p_buf->p_data = (uint8_t *)xrealloc( p_buf->p_data, p_buf->i_size );			// sunqueen modify
    }
...
#else
        p_out = (uint16_t *)ToCharset( "UTF-16LE", p_str, &i_out );			// sunqueen modify
#endif
    else
        p_out = NULL;
    if( p_out == NULL )
...
void var_buffer_initread( var_buffer_t *p_buf, void *p_data, int i_data )
{
    p_buf->i_size = i_data;
    p_buf->i_data = 0;
    p_buf->p_data = (uint8_t *)p_data;			// sunqueen modify
}
