...
static int NetFillBuffer( access_t *p_access )
{
    ...
    if( ( i_tcp > 0 ) && ufd[0].revents )
    {
        i_tcp_read =
            recv( p_sys->i_handle_tcp,
                  ...
                  i_tcp + MMS_BUFFER_SIZE/2, 0 );			// sunqueen modify
    }
    ...
    if( i_udp > 0 && ufd[i_tcp > 0].revents )
    {
        i_udp_read = recv( p_sys->i_handle_udp,
                           ...
                           i_udp + MMS_BUFFER_SIZE/2, 0 );			// sunqueen modify
    }
                              ...
                              int *pi_used )
{
 #define GET32( i_pos ) \
    ...
    free( p_sys->p_cmd );
    p_sys->i_cmd = i_data;
    p_sys->p_cmd = (uint8_t *)xmalloc( i_data );			// sunqueen modify
                             ...
                             int *pi_used )
{
    ...
    *pi_used = i_data; /* default */
    ...
    if( i_packet_id == p_sys->i_header_packet_id_type )
    {
        if( p_sys->p_header )
        {
            p_sys->p_header = (uint8_t *)xrealloc( p_sys->p_header,
                                      p_sys->i_header + i_packet_length - 8 );			// sunqueen modify
        ...
        {
            uint8_t* p_packet = (uint8_t *)xmalloc( i_packet_length - 8 ); // don't bother with preheader			// sunqueen modify
...
{
    access_t *p_access = (access_t *)p_data;			// sunqueen modify
