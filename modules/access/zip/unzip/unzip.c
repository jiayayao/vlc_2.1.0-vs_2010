...
local int strcmpcasenosensitive_internal (
    ...
    const char* fileName2)			// sunqueen modify
{
    for (;;)
    {
        char c1=*(fileName1++);
...
extern int ZEXPORT unzStringFileNameCompare (
    ...
    int iCaseSensitivity)			// sunqueen modify
{
...
local uLong unzlocal_SearchCentralDir(
    const zlib_filefunc_def* pzlib_filefunc_def,
    voidpf filestream)			// sunqueen modify
{
    unsigned char* buf;
    uLong uSizeFile;
    uLong uBackRead;
...
extern unzFile ZEXPORT unzOpen2 (
    const char *path,
    zlib_filefunc_def* pzlib_filefunc_def)			// sunqueen modify
{
    unz_s us;
    unz_s *s;
...
extern unzFile ZEXPORT unzOpen (
    const char *path)			// sunqueen modify
{
...
extern int ZEXPORT unzGetGlobalInfo (
    unzFile file,
    unz_global_info *pglobal_info)			// sunqueen modify
{
    unz_s* s;
    if (file==NULL)
        return UNZ_PARAMERROR;
...
local int unzlocal_GetCurrentFileInfoInternal (
    ...
    uLong extraFieldBufferSize,
    char *szComment,
    uLong commentBufferSize)			// sunqueen modify
{
    unz_s* s;
...
extern int ZEXPORT unzGetFilePos(
    unzFile file,
    unz_file_pos* file_pos)			// sunqueen modify
{
    unz_s* s;
...
local int unzlocal_CheckCurrentFileCoherencyHeader (
    ...
    uInt  *psize_local_extrafield)			// sunqueen modify
{
...
extern int ZEXPORT unzOpenCurrentFile3 (
    ...
    int* level,
    int raw,
    const char* password)			// sunqueen modify
{
    int err=UNZ_OK;
    uInt iSizeVar;
    unz_s* s;
...
#    endif
        ...
        (!raw))
    {
      ...
      pfile_in_zip_read_info->stream.next_in = (Bytef *)0;			// sunqueen modify
...
#    ifndef NOUNCRYPT
    ...
    {
        int i;
        s->pcrc_32_tab = (const unsigned long *)get_crc_table();			// sunqueen modify
...
extern int ZEXPORT unzGetGlobalComment (
    unzFile file,
    char *szComment,
    uLong uSizeBuf)			// sunqueen modify
{
    unz_s* s;
    uLong uReadThis ;
    if (file==NULL)
