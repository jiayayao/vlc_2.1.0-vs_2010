...
static int ProbeDVD( const char *psz_name )
{
    ...
    char iso_dsc[6];
    if( _lseek( fd, 0x8000 + 1, SEEK_SET ) == -1
     || _read( fd, iso_dsc, sizeof (iso_dsc) ) < sizeof (iso_dsc)
     || memcmp( iso_dsc, "CD001\x01", 6 ) )			// sunqueen modify
        goto bailout;

    /* Try to find the anchor (2 bytes at LBA 256) */
    uint16_t anchor;

    if( _lseek( fd, 256 * DVD_VIDEO_LB_LEN, SEEK_SET ) != -1
     && _read( fd, &anchor, 2 ) == 2
     && GetWLE( &anchor ) == 2 )			// sunqueen modify
        ret = VLC_SUCCESS; /* Found a potential anchor */
bailout:
    _close( fd );			// sunqueen modify
    return ret;
}
