...
static rmff_header_t *real_parse_sdp(char *data, char **stream_rules, uint32_t bandwidth) {
  ...
  header = (rmff_header_t *)calloc( 1, sizeof(rmff_header_t) );			// sunqueen modify
  ...
  header->streams = (rmff_mdpr_t **)calloc( desc->stream_count+1, sizeof(rmff_mdpr_t*) );			// sunqueen modify
...
rmff_header_t  *real_setup_and_get_header(rtsp_client_t *rtsp_session, int bandwidth) {
  ...
  char checksum[34];
  char *subscribe=NULL;
  char *buf = (char *)malloc(256);			// sunqueen modify
  if( !buf )
    return NULL;
  ...
  description = (char *)malloc(size+1);			// sunqueen modify
  if( !description )
    goto error;
  ...
  subscribe = (char *)malloc(256);			// sunqueen modify
  if( !subscribe )
    goto error;
  ...
  buf = (char *)realloc_or_free(buf, strlen(challenge2) + strlen(checksum) + 32);			// sunqueen modify
  if( !buf ) goto error;
  sprintf(buf, "RealChallenge2: %s, sd=%s", challenge2, checksum);
  rtsp_schedule_field(rtsp_session, buf);
  buf = (char *)realloc_or_free(buf, strlen(session_id) + 32);			// sunqueen modify
  if( !buf ) goto error;
  ...
  buf = (char *)realloc_or_free(buf, strlen(mrl) + 32);			// sunqueen modify
  if( !buf ) goto error;
