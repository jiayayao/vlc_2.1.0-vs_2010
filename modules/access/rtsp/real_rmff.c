...
int rmff_dump_header(rmff_header_t *h, void *buf_gen, int max) {
    uint8_t *buffer = (uint8_t *)buf_gen;			// sunqueen modify
...
rmff_fileheader_t *rmff_new_fileheader(uint32_t num_headers) {
  ...
  rmff_fileheader_t *fileheader = (rmff_fileheader_t *)calloc( 1, sizeof(rmff_fileheader_t) );			// sunqueen modify
  if( !fileheader )
    return NULL;
...
rmff_prop_t *rmff_new_prop (
  ...
  rmff_prop_t *prop = (rmff_prop_t *)calloc( 1, sizeof(rmff_prop_t) );			// sunqueen modify
  if( !prop )
    return NULL;
...
rmff_mdpr_t *rmff_new_mdpr(
  ...
  rmff_mdpr_t *mdpr = (rmff_mdpr_t *)calloc( 1, sizeof(rmff_mdpr_t) );			// sunqueen modify
  if( !mdpr )
    return NULL;
  ...
  mdpr->type_specific_data = (char *)malloc(type_specific_len);			// sunqueen modify
...
rmff_cont_t *rmff_new_cont(const char *title, const char *author, const char *copyright, const char *comment) {
  ...
  rmff_cont_t *cont = (rmff_cont_t *)calloc( 1, sizeof(rmff_cont_t) );			// sunqueen modify
  if( !cont )
    return NULL;
...
rmff_data_t *rmff_new_dataheader(uint32_t num_packets, uint32_t next_data_header) {
  rmff_data_t *data = (rmff_data_t *)calloc( 1, sizeof(rmff_data_t) );			// sunqueen modify
  if( !data )
    return NULL;
...
void rmff_fix_header(rmff_header_t *h) {
  ...
  if (!h->data) {
    ...
    h->data = (rmff_data_t *)calloc( 1, sizeof(rmff_data_t) );			// sunqueen modify
    if( h->data )
    {
  ...
  if (!h->fileheader) {
    ...
    h->fileheader = (rmff_fileheader_t *)calloc( 1, sizeof(rmff_fileheader_t) );			// sunqueen modify
    if( h->fileheader )
    {
