...
{
  char *psz_buffer = (char *)malloc( BUF_SIZE );			// sunqueen modify
...
static int rtsp_put( rtsp_client_t *rtsp, const char *psz_string )
{
    ...
    char *psz_buffer = (char *)malloc( i_buffer + 3 );			// sunqueen modify
    int i_ret;
                              ...
                              const char *psz_what )
{
    ...
    psz_buffer = (char *)malloc( strlen(psz_type) + strlen(psz_what) +
                         sizeof("RTSP/1.0") + 2 );			// sunqueen modify
...
static void rtsp_schedule_standard( rtsp_client_t *rtsp )
{
    ...
    {
        char *buf;
        buf = (char *)malloc( strlen(rtsp->p_private->session) + 15 );			// sunqueen modify
...
static int rtsp_get_answers( rtsp_client_t *rtsp )
{
    ...
    do { /* while we get answer lines */
      ...
      {
          char *buf = (char *)malloc( strlen(answer) );			// sunqueen modify
...
int rtsp_request_options( rtsp_client_t *rtsp, const char *what )
{
    ...
    {
        buf = (char *)malloc( strlen(rtsp->p_private->host) + 16 );			// sunqueen modify
...
int rtsp_request_describe( rtsp_client_t *rtsp, const char *what )
{
    ...
    else
    {
        buf = (char *)malloc( strlen(rtsp->p_private->host) +
                      strlen(rtsp->p_private->path) + 16 );			// sunqueen modify
...
int rtsp_read_data( rtsp_client_t *rtsp, uint8_t *buffer, unsigned int size )
{
    ...
    if( size >= 4 )
    {
            ...
            buffer[3]=='_' )
        {
            ...
            rest = (char *)malloc(17);			// sunqueen modify
                  ...
                  const char *psz_user_agent )
{
    ...
    if( !psz_mrl ) return -1;
    s = (rtsp_t *)malloc( sizeof(rtsp_t) );			// sunqueen modify
    rtsp->p_private = s;
    ...
    s->host = (char *)malloc(hostend+1);			// sunqueen modify
