...
static sdpplin_stream_t *sdpplin_parse_stream(char **data) {
  ...
  desc = (sdpplin_stream_t *)calloc( 1, sizeof(sdpplin_stream_t) );			// sunqueen modify
  if( !desc )
    return NULL;
  ...
  decoded = (unsigned char *)malloc( BUFLEN );			// sunqueen modify
  if( !decoded )
    goto error;
  ...
  *data=nl(*data);
  ...
  while (*data && **data && *data[0]!='m') {
    ...
    if(filter(*data,"a=OpaqueData:buffer;",&buf, BUFLEN)) {
      desc->mlti_data_size =
          vlc_b64_decode_binary_to_buffer((uint8_t *)decoded, BUFLEN, buf );			// sunqueen modify
      if ( desc->mlti_data_size ) {
          desc->mlti_data = (char *)malloc(desc->mlti_data_size);			// sunqueen modify
