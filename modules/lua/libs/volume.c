...
{
    playlist_t *p_this = vlclua_get_playlist_internal( L );
    long i_volume = cover_lroundf(playlist_VolumeGet( p_this ) * AOUT_VOLUME_DEFAULT);			// sunqueen modify
...
static int vlclua_volume_up( lua_State *L )
{
    ...
    lua_pushnumber( L, cover_lroundf(volume * AOUT_VOLUME_DEFAULT) );			// sunqueen modify
    return 1;
}
