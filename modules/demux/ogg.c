...
static void Ogg_ExtractXiphMeta( demux_t *p_demux, const void *p_headers, unsigned i_headers, unsigned i_skip )
{
    ...
    if( i_count >= 2 && pi_size[1] > i_skip )
    {
        ...
        {
            char psz_url[128];
            _snprintf( psz_url, sizeof(psz_url), "attachment://%s",
                p_ogg->attachments[i_cover_idx]->psz_name );			// sunqueen modify
                                   ...
                                   ogg_packet *p_oggpacket )
{
             ...
             !memcmp( p_oggpacket->packet, "AnxData", 7 ) )
    {
        ...
        {
            uint8_t *p = (uint8_t *)memchr( &p_oggpacket->packet[42], '\r',
                                 p_oggpacket->bytes - 1 );			// sunqueen modify
            if( p && p[0] == '\r' && p[1] == '\n' )
