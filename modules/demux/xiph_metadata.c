...
#define RM(x) do { i_data -= (x); p_data += (x); } while(0)
    ...
    _snprintf( psz_name, sizeof(psz_name), "picture%d", i_attachments );			// sunqueen modify
...
#define IF_EXTRACT(txt,var) \
    ...
    { \
        ...
        else if( !strncasecmp(psz_comment, "TRACKNUMBER=", strlen("TRACKNUMBER=" ) ) )
        {
            ...
            {
                char str[6];
                _snprintf(str, 6, "%d", u_track);			// sunqueen modify
                vlc_meta_Set( p_meta, vlc_meta_TrackNumber, str );
                _snprintf(str, 6, "%d", u_total);			// sunqueen modify
