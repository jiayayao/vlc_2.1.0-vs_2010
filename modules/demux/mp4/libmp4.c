...
static int MP4_ReadBox_ftyp( stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    {
        uint32_t *tab = p_box->data.p_ftyp->i_compatible_brands = (uint32_t *)
            calloc( p_box->data.p_ftyp->i_compatible_brands_count,
                    sizeof(uint32_t));			// sunqueen modify
...
static int MP4_ReadBox_tfrf(  stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_tfrf_data->p_tfrf_data_fields = (TfrfBoxDataFields_t *)calloc( p_tfrf_data->i_fragment_count,
                                              sizeof( TfrfBoxDataFields_t ) );			// sunqueen modify
...
static int MP4_ReadBox_trun(  stream_t *p_stream, MP4_Box_t *p_box )
{
    ...
    p_box->data.p_trun->p_samples = (MP4_descriptor_trun_sample_t *)
      calloc( p_box->data.p_trun->i_sample_count, sizeof(MP4_descriptor_trun_sample_t) );			// sunqueen modify
                                    ...
                                    MP4_Box_t *p_box, unsigned int i_level )
{
    ...
    else
    {
        ...
        if( MP4_BOX_TYPE_ASCII() )
            _snprintf( &str[i_level * 4], sizeof(str) - 4*i_level,
                      "+ %4.4s size %d",
                        (char*)&p_box->i_type, (uint32_t)p_box->i_size );			// sunqueen modify
        else
            _snprintf( &str[i_level * 4], sizeof(str) - 4*i_level,
                      "+ c%3.3s size %d",
                        (char*)&p_box->i_type+1, (uint32_t)p_box->i_size );			// sunqueen modify
        msg_Dbg( s, "%s", str );
    }
