...
#define SET(fct) do { char *psz_utf = strdup( p_0xa9xxx->data.p_0xa9xxx->psz_text ? p_0xa9xxx->data.p_0xa9xxx->psz_text : "" ); \
    if( psz_utf ) { EnsureUTF8( psz_utf );  \
                ...
                {
                    char psz_trck[11];
                    _snprintf( psz_trck, sizeof( psz_trck ), "%i",
                              p_0xa9xxx->data.p_trkn->i_track_number );			// sunqueen modify
                    vlc_meta_SetTrackNum( p_meta, psz_trck );
                    if( p_0xa9xxx->data.p_trkn->i_track_total > 0 )
                    {
                        _snprintf( psz_trck, sizeof( psz_trck ), "%i",
                                  p_0xa9xxx->data.p_trkn->i_track_total );			// sunqueen modify
                                   ...
                                   mp4_track_t *p_demux_track )
{
    ...
    p_demux_track->chunk = (mp4_chunk_t *)calloc( p_demux_track->i_chunk_count,
                                   sizeof( mp4_chunk_t ) );			// sunqueen modify
