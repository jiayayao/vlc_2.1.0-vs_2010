    ...
    case DEMUX_SET_GROUP:
    {
        ...
        else if( i_int < 0 )
        {
            ...
            if( p_list )
            {
                ...
                p_dst->p_values = (vlc_value_t *)calloc( p_list->i_count,
                                          sizeof(*p_dst->p_values) );			// sunqueen modify
                if( p_dst->p_values )
                {
                ...
                if( pE )
                {
                    ...
                    if( pE->i_text_length > 0 )
                    {
                        ...
                        if( psz_text )
                        {
                            ...
                            psz_extra = (char *)xrealloc( psz_extra,
                                   strlen(psz_extra) + strlen(psz_text) + 1 );			// sunqueen modify
                              ...
                              const dvbpsi_pmt_es_t *p_es )
{
    ...
    if( pid->es->fmt.i_extra_languages > 0 )
        pid->es->fmt.p_extra_languages =
            (extra_languages_t *)malloc( sizeof(*pid->es->fmt.p_extra_languages) *
                    pid->es->fmt.i_extra_languages );			// sunqueen modify
