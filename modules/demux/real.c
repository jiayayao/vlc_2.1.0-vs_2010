...
static int CodecAudioParse( demux_t *p_demux, int i_tk_id, const uint8_t *p_data, int i_data )
{
        ...
        fmt.i_codec == VLC_CODEC_ATRAC3 )
    {
        ...
        tk->p_subpackets = (block_t **)
            xcalloc( tk->i_subpackets, sizeof(block_t *) );			// sunqueen modify
        tk->p_subpackets_timecode = (mtime_t *)
            xcalloc( tk->i_subpackets , sizeof( int64_t ) );			// sunqueen modify
    }
