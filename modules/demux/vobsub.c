...
static int Open ( vlc_object_t *p_this )
{
    ...
    p_sys->i_tracks = 0;
    p_sys->track = (vobsub_track_t *)malloc( sizeof( vobsub_track_t ) );			// sunqueen modify
...
static int TextLoad( text_t *txt, stream_t *s )
{
    ...
    for( ;; )
    {
        ...
        ppsz_new = (char **)realloc( lines, (n + 1) * sizeof (char *) );			// sunqueen modify
        if( ppsz_new == NULL )
        {
            free( psz );
            break;
...
static int ParseVobSubIDX( demux_t *p_demux )
{
    ...
    for( ;; )
    {
        ...
        else if( !strncmp( "id:", line, 3 ) )
        {
            ...
            p_sys->i_tracks++;
            p_sys->track = (vobsub_track_t *)xrealloc( p_sys->track,
                    sizeof( vobsub_track_t ) * (p_sys->i_tracks + 1 ) );			// sunqueen modify
            ...
            current_tk->p_subtitles = (subtitle_t *)xmalloc( sizeof( subtitle_t ) );			// sunqueen modify
        ...
        else if( !strncmp( line, "timestamp:", 10 ) )
        {
                        ...
                        &h, &count, &m, &s, &ms, &loc ) >= 5  )
            {
                ...
                current_tk->p_subtitles = (subtitle_t *)
                    xrealloc( current_tk->p_subtitles,
                      sizeof( subtitle_t ) * (current_tk->i_subtitles + 1 ) );			// sunqueen modify
