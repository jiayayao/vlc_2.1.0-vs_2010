...
static int Demux_Seekable( demux_t *p_demux )
{
    ...
    for( i_track = 0; i_track < p_sys->i_track; i_track++ )
    {
        ...
        if( tk->i_samplesize )
        {
            toread[i_track].i_toread = AVI_PTSToByte( tk, cover_llabs( i_dpts ) );			// sunqueen modify
        }
        else
        {
            toread[i_track].i_toread = AVI_PTSToChunk( tk, cover_llabs( i_dpts ) );			// sunqueen modify
        }
...
#define p_stream    p_sys->track[avi_pk.i_stream]
        ...
        {
            /* check for time */
            if( cover_llabs( AVI_GetPTS( p_stream ) -
                        AVI_GetPTS( p_stream_master ) )< 600*1000 )			// sunqueen modify
            {
                              ...
                              avi_entry_t *p_entry )
{
    ...
    {
        p_index->i_max += 16384;
        p_index->p_entry = (avi_entry_t *)realloc_or_free( p_index->p_entry,
                                            p_index->i_max * sizeof( *p_index->p_entry ) );			// sunqueen modify
        if( !p_index->p_entry )
            return;
    }
    ...
    for( ;; )
    {
        ...
        else
        {
            ...
            case AVIFOURCC_idx1:
                ...
                {
                    avi_chunk_list_t *p_sysx;
                    p_sysx = (avi_chunk_list_t *)AVI_ChunkFind( &p_sys->ck_root,
                                            AVIFOURCC_RIFF, 1 );			// sunqueen modify
                          ...
                          avi_chunk_list_t *p_riff, avi_chunk_avih_t *p_avih )
{
    ...
    char buffer[200];
    _snprintf( buffer, sizeof(buffer), "%s%s%s%s",
              p_avih->i_flags&AVIF_HASINDEX      ? " HAS_INDEX"      : "",
              p_avih->i_flags&AVIF_MUSTUSEINDEX  ? " MUST_USE_INDEX" : "",
              p_avih->i_flags&AVIF_ISINTERLEAVED ? " IS_INTERLEAVED" : "",
              p_avih->i_flags&AVIF_TRUSTCKTYPE   ? " TRUST_CKTYPE"   : "" );			// sunqueen modify
