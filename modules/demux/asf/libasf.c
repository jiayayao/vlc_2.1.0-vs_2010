...
static int ASF_ReadObject_Index( stream_t *s, asf_object_t *p_obj )
{
    ...
    p_index->index_entry = (asf_index_entry_t *)calloc( p_index->i_index_entry_count,
                                   sizeof(asf_index_entry_t) );			// sunqueen modify
...
static int ASF_ReadObject_metadata( stream_t *s, asf_object_t *p_obj )
{
    ...
    p_meta->record = (asf_metadata_record_t *)calloc( p_meta->i_record_entries_count,
                             sizeof(asf_metadata_record_t) );			// sunqueen modify
    if( !p_meta->record )
        return VLC_ENOMEM;
...
static int ASF_ReadObject_codec_list( stream_t *s, asf_object_t *p_obj )
{
    ...
    {
        p_cl->codec = (asf_codec_entry_t *)calloc( p_cl->i_codec_entries_count,
                              sizeof( asf_codec_entry_t ) );			// sunqueen modify
        if( !p_cl->codec )
            return VLC_ENOMEM;
                                                      ...
                                                      asf_object_t *p_obj)
{
    ...
    p_esp->pi_stream_name_language = (int *)calloc( p_esp->i_stream_name_count,
                                             sizeof(int) );			// sunqueen modify
    p_esp->ppsz_stream_name = (char **)calloc( p_esp->i_stream_name_count,
                                      sizeof(char*) );			// sunqueen modify
...
static int ASF_ReadObject_marker(stream_t *s, asf_object_t *p_obj)
{
    ...
    if( p_mk->i_count > 0 )
    {
        p_mk->marker = (asf_marker_t *)calloc( p_mk->i_count,
                              sizeof( asf_marker_t ) );			// sunqueen modify
        if( !p_mk->marker )
            return VLC_ENOMEM;
                                 ...
                                 asf_object_common_t *p_node, unsigned i_level )
{
    ...
    _snprintf( &str[4*i_level], sizeof(str) - 5*i_level,
             "+ '%s'"
#ifdef ASF_DEBUG
...
asf_object_root_t *ASF_ReadObjectRoot( stream_t *s, int b_seekable )
{
    ...
    {
        p_root->p_fp = (asf_object_file_properties_t *)ASF_FindObject( p_root->p_hdr,
                                       &asf_object_file_properties_guid, 0 );			// sunqueen modify

        if( p_root->p_fp )
        {
            asf_object_t *p_hdr_ext =
                (asf_object_t *)ASF_FindObject( p_root->p_hdr,
                                &asf_object_header_extension_guid, 0 );			// sunqueen modify
            if( p_hdr_ext )
            {
                int i_ext_stream;

                p_root->p_metadata = (asf_object_metadata_t *)
                    ASF_FindObject( p_hdr_ext,
                                    &asf_object_metadata_guid, 0 );			// sunqueen modify
                ...
                {
                    asf_object_t *p_esp = (asf_object_t *)
                        ASF_FindObject( p_hdr_ext,
                                   &asf_object_extended_stream_properties_guid, i );			// sunqueen modify
