...
static int DemuxInit( demux_t *p_demux )
{
    ...
    asf_object_index_t *p_index = (asf_object_index_t *)ASF_FindObject( p_sys->p_root,
                                                  &asf_object_simple_index_guid, 0 );			// sunqueen modify
    const bool b_index = p_index && p_index->i_index_entry_count;

    /* Find the extended header if any */
    asf_object_t *p_hdr_ext = (asf_object_t *)ASF_FindObject( p_sys->p_root->p_hdr,
    ...
    for( unsigned i_stream = 0; i_stream < p_sys->i_track; i_stream++ )
    {
        ...
        p_sp = (asf_object_stream_properties_t *)ASF_FindObject( p_sys->p_root->p_hdr,
                               &asf_object_stream_properties_guid,
                               i_stream );			// sunqueen modify
        p_esp = NULL;
        ...
        if( p_hdr_ext )
        {
            ...
            {
                asf_object_t *p_tmp = (asf_object_t *)
    ...
    if( ( p_cd = (asf_object_content_description_t *)ASF_FindObject( p_sys->p_root->p_hdr,
                                 &asf_object_content_description_guid, 0 ) ) )			// sunqueen modify
    {
