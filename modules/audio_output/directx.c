 ...
 *****************************************************************************/
static int Start( audio_output_t *p_aout, audio_sample_format_t *__restrict fmt )			// sunqueen modify
{
    char * psz_speaker;
    int i = 0;
...
static int VolumeSet( audio_output_t *p_aout, float volume )
{
    ...
    LONG mb = cover_lroundf( 6000.f * log10f( __MIN( volume, 1.f ) ));			// sunqueen modify
...
static int InitDirectSound( audio_output_t *p_aout )
{
    ...
    OurDirectSoundCreate = (HRESULT (__stdcall *)(LPGUID,LPDIRECTSOUND *,LPUNKNOWN))
        GetProcAddress( p_aout->sys->hdsound_dll,
                        "DirectSoundCreate" );			// sunqueen modify
                           ...
                           bool b_probe )
{
    ...
    if( IDirectSoundBuffer_QueryInterface( p_aout->sys->p_dsbuffer,
                                           ...
                                           (void **) &p_aout->sys->p_notify )			// sunqueen modify
            != DS_OK )
    {
