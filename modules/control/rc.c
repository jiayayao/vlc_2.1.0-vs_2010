...
//    char fmt_eol[strlen (psz_fmt) + 3];
    ...
    _snprintf (fmt_eol, sizeof (fmt_eol), "%s\r\n", psz_fmt);			// sunqueen modify
    ...
    vlc_value_t oldval, vlc_value_t newval, void *p_data )
{
    ...
    msg_rc( STATUS_CHANGE "( audio volume: %ld )",
            cover_lroundf(newval.f_float * AOUT_VOLUME_DEFAULT) );			// sunqueen modify
                     ...
                     vlc_value_t oldval, vlc_value_t newval, void *p_data )
{
    ...
    else if( !strcmp( psz_cmd, "status" ) )
    {
        ...
        if( volume >= 0.f )
            msg_rc( STATUS_CHANGE "( audio volume: %ld )",
                    cover_lroundf(volume * AOUT_VOLUME_DEFAULT) );			// sunqueen modify
                   ...
                   vlc_value_t oldval, vlc_value_t newval, void *p_data )
{
    ...
    else
    {
        /* Get. */
        msg_rc( STATUS_CHANGE "( audio volume: %ld )",
               cover_lroundf( playlist_VolumeGet( p_playlist ) * AOUT_VOLUME_DEFAULT ) );			// sunqueen modify
        i_error = VLC_SUCCESS;
    }
