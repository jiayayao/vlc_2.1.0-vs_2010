...
static void *Thread( void *p_data )
{
    ...
    vlc_mutex_lock( &p_sys->lock );
    p_sys->hotkeyWindow =
        (HWND)CreateWindow( _T("STATIC"),            /* name of window class */
                ...
                NULL );                                 /* sent to WM_CREATE */			// sunqueen modify
    ...
    for( const struct hotkey *p_hotkey = (hotkey *)p_intf->p_libvlc->p_hotkeys;			// sunqueen modify
