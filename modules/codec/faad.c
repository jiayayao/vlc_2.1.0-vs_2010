...
vlc_module_begin ( faad )				// sunqueen modify
...
static int Open( vlc_object_t *p_this )
{
    ...
    if( p_dec->fmt_in.i_extra > 0 )
    {
        ...
        if( faacDecInit2( p_sys->hfaad, (unsigned char *)p_dec->fmt_in.p_extra,
                          p_dec->fmt_in.i_extra,
                          &i_rate, &i_channels ) < 0 )			// sunqueen modify
        {
...
static block_t *DecodeBlock( decoder_t *p_dec, block_t **pp_block )
{
    ...
    if( p_sys->i_buffer )
    {
        ...
        DoReordering( (uint32_t *)p_out->p_buffer, (uint32_t *)samples,
                      frame.samples / nbChannels, nbChannels,
                      p_sys->pi_channel_positions );			// sunqueen modify
