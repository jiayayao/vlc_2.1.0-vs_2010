...
static int OpenEncoder( vlc_object_t *p_this )
{
    ...
    p_sys->p_buffer = (uint8_t *)xmalloc(p_sys->i_frame_samples
                            * p_enc->fmt_in.audio.i_channels * 16);			// sunqueen modify
...
static block_t *EncodeFrames( encoder_t *p_enc, block_t *p_aout_buf )
{
    ...
    for ( int i = 0; i < i_num_frames; ++i )
    {
        ...
        swab( (char *)(p_aout_buf->p_buffer + i_bytes_consumed), (char *)(frame + 6 + i_kept_bytes),
              i_consume_bytes );			// sunqueen modify
#endif
