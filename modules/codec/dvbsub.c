...
static void decode_page_composition( decoder_t *p_dec, bs_t *s )
{
    ...
    p_sys->p_page->p_region_defs = (dvbsub_regiondef_t *)
        malloc( p_sys->p_page->i_region_defs * sizeof(dvbsub_regiondef_t) );			// sunqueen modify
...
static void decode_region_composition( decoder_t *p_dec, bs_t *s )
{
    ...
    while( i_processed_length < i_segment_length )
    {
        ...
        p_region->p_object_defs = (dvbsub_objectdef_t *)xrealloc( p_region->p_object_defs,
                     sizeof(dvbsub_objectdef_t) * p_region->i_object_defs );			// sunqueen modify
    ...
    else
    {
             ...
             p_region = p_region->p_next )
        {
            for( i = 0; i < p_region->i_object_defs; i++ )
            {
                ...
                p_region->p_object_defs[i].psz_text = (char *)
                    xrealloc( p_region->p_object_defs[i].psz_text,
                             i_number_of_codes + 1 );			// sunqueen modify
...
static subpicture_t *YuvaYuvp( subpicture_t *p_subpic )
{
    ...
    for( p_region = p_subpic->p_region; p_region; p_region = p_region->p_next )
    {
        ...
        pi_delta = (int *)xmalloc( ( p_region->p_picture->p[0].i_pitch + 1 )
                            * sizeof(int) * 4  );			// sunqueen modify
                                     ...
                                     subpicture_t *p_subpic )
{
         ...
         p_region; p_region = p_region->p_next, i_regions++ )
    {
        if( i_regions >= p_sys->i_regions )
        {
            ...
            p_sys->p_regions = (encoder_region_t *)xrealloc( p_sys->p_regions,
                          sizeof(encoder_region_t) * (p_sys->i_regions + 1) );			// sunqueen modify
