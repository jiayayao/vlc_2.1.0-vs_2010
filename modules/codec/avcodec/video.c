...
static void ffmpeg_InitCodec( decoder_t *p_dec )
{
    ...
    if( p_sys->i_codec_id == AV_CODEC_ID_SVQ3 )
    {
        ...
        p = p_sys->p_context->extradata  = (uint8_t *)
            malloc( p_sys->p_context->extradata_size );			// sunqueen modify
        if( !p )
            return;
    ...
    else
    {
        ...
        p_sys->p_context->extradata = (uint8_t *)
            malloc( i_size + FF_INPUT_BUFFER_PADDING_SIZE );			// sunqueen modify
