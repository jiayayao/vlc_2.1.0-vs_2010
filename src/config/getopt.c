...
static void exchange(char **argv, vlc_getopt_t *__restrict state)			// sunqueen modify
{
...
int vlc_getopt_long(int argc, char *const *argv,
                    ...
                    const struct vlc_option *__restrict longopts, int *longind,
                    vlc_getopt_t *__restrict state)			// sunqueen modify
{
    state->arg = NULL;
...
#define NONOPTION_P (argv[state->ind][0] != '-' || argv[state->ind][1] == '\0')
...

    {
        ...
        char *temp = strchr((char *)optstring, c);			// sunqueen modify
