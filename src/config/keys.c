...
{
    const char *sa = (const char *)key, *sb = (const char *)elem;			// sunqueen modify
    return strcmp (sa, sb);
}
...
uint_fast32_t vlc_str2keycode (const char *name)
{
    ...
    key_descriptor_t *d = (key_descriptor_t *)bsearch (name, vlc_keys, KEYS_COUNT,
                                   sizeof (vlc_keys[0]), keystrcmp);			// sunqueen modify
    if (d != NULL)
        code = d->i_key_code;
    else
...
{
    const struct mapping *ka = (const mapping *)a, *kb = (const mapping *)b;			// sunqueen modify
...
{
    void *const *map = (void *const *)d;			// sunqueen modify
    const struct mapping **pent;
    uint32_t keycode = curkey.i_int;

    pent = (const mapping **)cover_tfind (&keycode, map, keycmp);			// sunqueen modify
    if (pent == NULL)
        return VLC_SUCCESS;
...
{
    struct mapping *entry = (mapping *)malloc (sizeof (*entry));			// sunqueen modify
    if (entry == NULL)
        return ENOMEM;
    entry->key = keycode;
    entry->action = action;

    struct mapping **pent = (mapping **)cover_tsearch (entry, map, keycmp);			// sunqueen modify
...
struct vlc_actions *vlc_InitActions (libvlc_int_t *libvlc)
{
    ...
    struct hotkey *keys;
    struct vlc_actions *as = (vlc_actions *)malloc (sizeof (*as) + (ACTIONS_COUNT + 1) * sizeof (*keys));			// sunqueen modify
    ...
    for (size_t i = 0; i < ACTIONS_COUNT; i++)
    {
        ...
        _snprintf (name, sizeof (name), "global-key-%s", actions[i].name);			// sunqueen modify
...
{
    const struct action *act = (const action *)ent;			// sunqueen modify
    return strcmp((const char *)key, act->name);			// sunqueen modify
}
...
vlc_action_t vlc_GetActionId (const char *name)
{
    ...
    act = (const action *)bsearch(name, actions, ACTIONS_COUNT, sizeof(*act), actcmp);			// sunqueen modify
