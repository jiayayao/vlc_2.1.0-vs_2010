                                 ...
                                 int input_rate)
{
    ...
    if (cover_llabs (drift) > 2 * owner->sync.resamp_start_drift)			// sunqueen modify
    ...
    if (2 * cover_llabs (drift) <= owner->sync.resamp_start_drift)			// sunqueen modify
...
int aout_DecPlay (audio_output_t *aout, block_t *block, int input_rate)
{
	...
	// 计算block的pts，太早或者太晚都会导致该block被丢弃
