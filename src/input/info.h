...
{
    info_t *info = (info_t *)malloc(sizeof(*info));			// sunqueen modify
    if (!info)
        return NULL;
...
{
    info_category_t *cat = (info_category_t *)malloc(sizeof(*cat));			// sunqueen modify
    if (!cat)
        return NULL;
                                             ...
                                             info_t *info)
{
    ...
    if (old) {
        ...
        INSERT_ELEM( (info_t **), cat->pp_infos, cat->i_infos, cat->i_infos, info);			// sunqueen modify
    }
}
...
static inline int info_category_DeleteInfo(info_category_t *cat, const char *name)
{
    ...
    if (info_category_FindInfo(cat, &index, name)) {
        info_Delete(cat->pp_infos[index]);
        REMOVE_ELEM( (info_t **), cat->pp_infos, cat->i_infos, index);			// sunqueen modify
        return VLC_SUCCESS;
    }
    return VLC_EGENERIC;
}
