...
{
    demux_t *p_demux = (demux_t *)vlc_custom_create( p_obj, sizeof( *p_demux ), "demux" );			// sunqueen modify
    ...
    /* Parse URL */
    p_demux->psz_access = _strdup( psz_access );			// sunqueen modify
    p_demux->psz_demux  = _strdup( psz_demux );			// sunqueen modify
    p_demux->psz_location = _strdup( psz_location );			// sunqueen modify
    ...
    if( *p_demux->psz_demux == '\0' )
    {
        ...
        if( p_demux->psz_demux == NULL )
            p_demux->psz_demux = _strdup( "" );			// sunqueen modify
    }
...
{
    return p_demux->p_input ? (input_thread_t *)vlc_object_hold((vlc_object_t*)p_demux->p_input) : NULL;			// sunqueen modify
}
...
decoder_t *demux_PacketizerNew( demux_t *p_demux, es_format_t *p_fmt, const char *psz_msg )
{
    ...
    p_packetizer = (decoder_t *)vlc_custom_create( p_demux, sizeof( *p_packetizer ),
                                      "demux packetizer" );			// sunqueen modify
    if( !p_packetizer )
    {
