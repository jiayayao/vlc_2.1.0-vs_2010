    ...
    switch( i_query )
    {
        ...
        case INPUT_GET_BOOKMARKS:
            ...
            if( p_input->p->i_bookmark )
            {
                ...
                *ppp_bkmk = (seekpoint_t **) malloc( sizeof(seekpoint_t *) *
                                    p_input->p->i_bookmark );			// sunqueen modify
...
static void UpdateBookmarksOption( input_thread_t *p_input )
{
    ...
    for( int i = 0; i < p_input->p->i_bookmark; i++ )
    {
        ...
        i_len += _snprintf( NULL, 0, psz_format,
                           p_bookmark->psz_name,
                           p_bookmark->i_byte_offset,
                           p_bookmark->i_time_offset/1000000 );			// sunqueen modify
    }
