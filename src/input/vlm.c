...
vlm_t *vlm_New ( vlc_object_t *p_this )
{
    ...
    p_vlm = (vlm_t *)vlc_custom_create( p_this->p_libvlc, sizeof( *p_vlm ),
                               "vlm daemon" );			// sunqueen modify
    if( !p_vlm )
    {
                    ...
                    if( i_real_date > i_lastcheck )
                    {
                        for( j = 0; j < vlm->schedule[i]->i_command; j++ )
                        {
                            TAB_APPEND( (char **),
                                        ...
                                        strdup(vlm->schedule[i]->command[j] ) );				// sunqueen modify
                        }
                    }
                }
...
static int vlm_ControlMediaAdd( vlm_t *p_vlm, vlm_media_t *p_cfg, int64_t *p_id )
{
    ...
    if( p_cfg->b_vod && !p_vlm->p_vod )
    {
        p_vlm->p_vod = (vod_t *)vlc_custom_create( VLC_OBJECT(p_vlm), sizeof( vod_t ),
                                          "vod server" );				// sunqueen modify
