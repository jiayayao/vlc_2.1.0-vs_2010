                                     ...
                                     bool b_delete )
{
    ...
    {
        char psz_page[3+1];
        _snprintf( psz_page, sizeof(psz_page), "%d%2.2x",
                  fmt->subs.teletext.i_magazine,
                  fmt->subs.teletext.i_page );			// sunqueen modify
...
static void EsOutUpdateInfo( es_out_t *out, es_out_id_t *es, const es_format_t *fmt, const vlc_meta_t *p_meta )
{
    ...
    /* Create category */
    char psz_cat[128];
    _snprintf( psz_cat, sizeof(psz_cat),_("Stream %d"), es->i_meta_id );				// sunqueen modify
    ...
    case VIDEO_ES:
       ...
       {
           div = cover_lldiv( (float)fmt->video.i_frame_rate /
                               fmt->video.i_frame_rate_base * 1000000,
                               1000000 );			// sunqueen modify
           if( div.rem > 0 )
