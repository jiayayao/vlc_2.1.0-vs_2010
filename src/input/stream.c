...
static int AStreamControl( stream_t *s, int i_query, va_list args )
{
    ...
    switch( i_query )
    {
        case STREAM_GET_SIZE:
            pi_64 = (uint64_t *)va_arg( args, uint64_t * );			// sunqueen modify
            if( s->p_sys->i_list )
            {
                int i;
                *pi_64 = 0;
        ...
        case STREAM_SET_POSITION:
            i_64 = (uint64_t)va_arg( args, uint64_t );			// sunqueen modify
            switch( p_sys->method )
            {
...
static int AReadStream( stream_t *s, void *p_read, unsigned int i_read )
{
    ...
    i_read = p_sys->p_list_access->pf_read( p_sys->p_list_access, (uint8_t *)p_read,
                                            i_read );			// sunqueen modify
