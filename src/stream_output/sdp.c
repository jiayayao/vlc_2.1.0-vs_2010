...
//        va_copy (aq, ap);
		aq = ap;			// sunqueen modify
        addlen += 1 + vsnprintf (NULL, 0, fmt, aq);
        va_end (aq);
    }

    char *ret = (char *)realloc (*sdp, oldlen + addlen);			// sunqueen modify
    if (ret == NULL)
        return NULL;
                    ...
                    const char *fmtp)
{
    ...
    outlen += _snprintf (NULL, 0,
                        "m=%s %u %s %d\r\n"
                        "b=TIAS:%u\r\n"
                        "b=RR:0\r\n",
                        type, dport, protocol, pt, bw);			// sunqueen modify

    newsdp = (char *)realloc (*sdp, outlen + 1);			// sunqueen modify
    if (newsdp == NULL)
        return NULL;
