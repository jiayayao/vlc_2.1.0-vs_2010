                                 ...
                                 const struct addrinfo *ptr)
{
	...
	int size_off = 0x80000;			// sunqueen add
    setsockopt (fd, SOL_SOCKET, SO_RCVBUF,
                (const char *)&size_off/*(void *)&(int){ 0x80000 }*/, sizeof (int));			// sunqueen modify
...
//            setsockopt (fd, SOL_IPV6, IPV6_V6ONLY, &(int){ 0 }, sizeof (int));
			int opt = 0;
            setsockopt (fd, SOL_IPV6, IPV6_V6ONLY, (const char *)&opt, sizeof (int));
		}
		//  sunqueen modify end
#endif
                                 ...
                                 int fd, int family, int hlim )
{
    ...
    if( setsockopt( fd, proto, cmd, (const char *)&hlim, sizeof( hlim ) ) < 0 )			// sunqueen modify
    {
        /* BSD compatibility */
        unsigned char buf;
        ...
        if( setsockopt( fd, proto, cmd, (const char *)&buf, sizeof( buf ) ) )			// sunqueen modify
        {
    ...
    switch (family)
    {
        ...
        case AF_INET6:
            if (setsockopt (fd, SOL_IPV6, IPV6_MULTICAST_IF,
                            (const char *)&scope, sizeof (scope)) == 0)			// sunqueen modify
                return 0;
            break;
#endif
                     ...
                     const struct sockaddr *grp, socklen_t grplen)
{
    ...
    if (setsockopt (fd, level, MCAST_JOIN_SOURCE_GROUP,
                    (const char *)&gsr, sizeof (gsr)) == 0)			// sunqueen modify
        return 0;
 ...
 * so ignore it to use the same code as on 10.5 or 10.6 */
    ...
    if (setsockopt (fd, level, MCAST_JOIN_GROUP, (const char *)&gr, sizeof (gr)) == 0)			// sunqueen modify
        return 0;
...
//    if( getsockname( fd, (struct sockaddr *)&addr, &(socklen_t){ sizeof (addr) }) )
	socklen_t size_off = sizeof (addr);
    if( getsockname( fd, (struct sockaddr *)&addr, (int *)&size_off) )
    // sunqueen modify end
        return -1;
    ...
    return setsockopt( fd, level, cmd, (const char *)&dscp/*(int){ dscp }*/, sizeof (int));		// sunqueen modify
}
                      ...
                      int i_hlim, int proto )
{
    ...
    for (struct addrinfo *ptr = res; ptr != NULL; ptr = ptr->ai_next)
    {
        ...
        * to avoid packet loss caused by scheduling problems */
		int size_off = 0x80000;			// sunqueen add
        setsockopt (fd, SOL_SOCKET, SO_RCVBUF, (const char *)&size_off/*&(int){ 0x80000 }*/, sizeof (int));			// sunqueen modify
        setsockopt (fd, SOL_SOCKET, SO_SNDBUF, (const char *)&size_off/*&(int){ 0x80000 }*/, sizeof (int));			// sunqueen modify

        /* Allow broadcast sending */
		int offset = 1;			// sunqueen add
        setsockopt (fd, SOL_SOCKET, SO_BROADCAST, (const char *)&offset/*&(int){ 1 }*/, sizeof (int));			// sunqueen modify
...
int net_SetCSCov (int fd, int sendcov, int recvcov)
{
	...
	socklen_t offset = sizeof (type);			// sunqueen add
    if (getsockopt (fd, SOL_SOCKET, SO_TYPE,
                    (char *)&type, (int *)&offset/*&(socklen_t){ sizeof (type) }*/))			// sunqueen modify
        return VLC_EGENERIC;
