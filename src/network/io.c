          ...
          void *__restrict p_buf, size_t i_buflen, bool waitall)			// sunqueen modify
{
    ...
    ufd[0].fd = fd;
	ufd[0].events  = __POLLIN;			// sunqueen modify
    ufd[1].fd = vlc_object_waitpipe (p_this);
	ufd[1].events  = __POLLIN;			// sunqueen modify
...
//    };
    ...
    ufd[0].fd      = fd;
	ufd[0].events  = __POLLOUT;			// sunqueen modify
	ufd[0].revents = 0;
    ufd[1].fd      = vlc_object_waitpipe (p_this);
	ufd[1].events  = __POLLIN;			// sunqueen modify
	ufd[1].revents = 0;
// sunqueen modify end
    ...
    while( i_data > 0 )
    {
        ...
        {   /* If POLLHUP resp. POLLERR|POLLNVAL occurs while we have already
            ...
            if (ufd[0].revents & (__POLLHUP|__POLLERR|__POLLNVAL))			// sunqueen modify
                break;
