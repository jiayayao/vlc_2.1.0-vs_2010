                                  ...
                                  void *p_buffer_allocation_data )
{
    ...
    filter_chain_t *p_chain = (filter_chain_t *)malloc( size );				// sunqueen modify
    if( !p_chain )
        return NULL;
...
static filter_t *filter_chain_AppendFilterInternal( filter_chain_t *p_chain,
                                                    ...
                                                    const es_format_t *p_fmt_out )
{
    chained_filter_t *p_chained = (chained_filter_t *)
        vlc_custom_create( p_chain->p_this, sizeof(*p_chained), "filter" );				// sunqueen modify
    ...
    vlc_mouse_t *p_mouse = (vlc_mouse_t *)malloc( sizeof(*p_mouse) );				// sunqueen modify
    if( p_mouse )
