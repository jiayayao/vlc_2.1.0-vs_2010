        ...
        vlc_event_type_t event_type )
{
    ...
    listeners_group = (vlc_event_listeners_group_t *)malloc(sizeof(vlc_event_listeners_group_t));			// sunqueen modify
                     ...
                     vlc_event_t * p_event )
{
    ...
    FOREACH_ARRAY( listeners_group, p_em->listeners_groups )
        if( listeners_group->event_type == p_event->type )
        {
            ...
            array_of_cached_listeners = (vlc_event_listener_t *)malloc(
                    sizeof(vlc_event_listener_t)*i_cached_listeners );			// sunqueen modify
                      ...
                      void *p_user_data )
{
    vlc_event_listeners_group_t * listeners_group;
    vlc_event_listener_t * listener;
    listener = (vlc_event_listener_t *)malloc(sizeof(vlc_event_listener_t));				// sunqueen modify
    if( !listener )
        return VLC_ENOMEM;
        ...
        if( listeners_group->event_type == event_type )
        {
            FOREACH_ARRAY( listener, listeners_group->listeners )
                    ...
                    listener->p_user_data == p_user_data )
                {
                    ...
                    /* that's our listener */
                    ARRAY_REMOVE( (vlc_event_listener_t **), listeners_group->listeners,
                        fe_idx /* This comes from the macro (and that's why
                                  I hate macro) */ );			// sunqueen modify
                    free( listener );
                    vlc_mutex_unlock( &p_em->event_sending_lock );
                    vlc_mutex_unlock( &p_em->object_lock );
                    return;
