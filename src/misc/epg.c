...
{
    vlc_epg_event_t *p_evt = (vlc_epg_event_t*)malloc( sizeof(*p_evt) );			// sunqueen modify
    if( !p_evt )
        return;
...
{
    vlc_epg_t *p_epg = (vlc_epg_t*)malloc( sizeof(*p_epg) );			// sunqueen modify
    if( p_epg )
...
void vlc_epg_Merge( vlc_epg_t *p_dst, const vlc_epg_t *p_src )
{
    ...
    for( i = 0; i < p_src->i_event; i++ )
    {
        ...
        if( b_add )
        {
            vlc_epg_event_t *p_copy = (vlc_epg_event_t *)calloc( 1, sizeof(vlc_epg_event_t) );				// sunqueen modify
            if( !p_copy )
                break;
