                         ...
                         const char *psz_prefix )
{
    ...
    {
        _snprintf( psz_name, 49, " \"%s\"", priv->psz_name );				// sunqueen modify
        if( psz_name[48] )
            psz_name[48] = '\"';
    }
    vlc_mutex_unlock (&name_lock);

    _snprintf( psz_refcount, 19, ", %u refs", atomic_load( &priv->refs ) );				// sunqueen modify

    psz_parent[0] = '\0';
    /* FIXME: need structure lock!!! */
    if( vlc_externals(priv)->p_parent )
        _snprintf( psz_parent, 19, ", parent %p",
                  vlc_externals(priv)->p_parent );				// sunqueen modify
