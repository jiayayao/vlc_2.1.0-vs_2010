...
char *mstrtime( char *psz_buffer, mtime_t date )
{
    ...
    _snprintf( psz_buffer, MSTRTIME_MAX_SIZE, "%02d:%02d:%02d-%03d.%03d",
             ...
             (int) (date % ll1000) );			// sunqueen modify
    return( psz_buffer );
}
...
char *secstotimestr( char *psz_buffer, int32_t i_seconds )
{
    ...
    if( d.quot )
        _snprintf( psz_buffer, MSTRTIME_MAX_SIZE, "%u:%02u:%02u",
                 d.quot, d.rem, i_seconds );				// sunqueen modify
    else
        _snprintf( psz_buffer, MSTRTIME_MAX_SIZE, "%02u:%02u",
                  d.rem, i_seconds );				// sunqueen modify
    return psz_buffer;
}
...
mtime_t date_Increment( date_t *p_date, uint32_t i_nb_samples )
{
    ...
    lldiv_t d = cover_lldiv( i_dividend, p_date->i_divider_num );			// sunqueen modify
