...
static bool GetUpdateFile( update_t *p_update )
{
    ...
    psz_update_data = (char *)malloc( i_read + 1 ); /* terminating '\0' */			// sunqueen modify
    if( !psz_update_data )
        goto error;
    ...
    if( !(psz_version_line = (char *)malloc( i_len + 1)) )			// sunqueen modify
        goto error;
    ...
    if( !(p_update->release.psz_url = (char *)malloc( i_len + 1)) )			// sunqueen modify
        goto error;
    ...
    if( !(p_update->release.psz_desc = (char *)malloc( i_len + 1)) )			// sunqueen modify
        goto error;
...
void update_Check( update_t *p_update, void (*pf_callback)( void*, bool ), void *p_data )
{
    ...
    update_check_thread_t *p_uct = (update_check_thread_t *)calloc( 1, sizeof( *p_uct ) );			// sunqueen modify
    if( !p_uct ) return;
...
void update_Download( update_t *p_update, const char *psz_destdir )
{
    ...
    update_download_thread_t *p_udt = (update_download_thread_t *)
        vlc_custom_create( p_update->p_libvlc, sizeof( *p_udt ),
                           "update download" );			// sunqueen modify
    if( !p_udt )
        return;
