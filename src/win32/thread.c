...
static mtime_t mdate_perf (void)
{
    ...
    lldiv_t d = cover_lldiv (counter.QuadPart, clk.perf.freq.QuadPart);			// sunqueen modify
...
static void SelectClockSource (vlc_object_t *obj)
{
    ...
    if (!strcmp (name, "interrupt"))
    {
        ...
        if (unlikely(h == NULL))
            abort ();
        clk.interrupt.query = (BOOL (__cdecl *)(PULONGLONG))GetProcAddress ((HMODULE)h,
                                                      "QueryUnbiasedInterruptTime");			// sunqueen modify
