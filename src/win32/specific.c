...
#   define ABOVE_NORMAL_PRIORITY_CLASS 0x00008000
       ...
       && var_InheritBool( p_this, "started-from-file" ) ) )
    {
        ...
        if( !( hmutex = CreateMutex( 0, TRUE, /*L*/"VLC ipc "VERSION ) ) )			// sunqueen modify
        {
        ...
        if( GetLastError() != ERROR_ALREADY_EXISTS )
        {
            ...
            p_helper = (vlc_object_t *)
                vlc_custom_create( p_this, sizeof(*p_helper), "ipc helper" );		// sunqueen modify
        ...
        else
        {
             ...
             * 1st instance */
            if( !( ipcwindow = FindWindow( 0, /*L*/"VLC ipc "VERSION ) ) )			// sunqueen modify
            {
                msg_Err( p_this, "one instance mode DISABLED "
            ...
            if( i_argc > 0 )
            {
                ...
                p_data = (vlc_ipc_data_t *)malloc( i_data );			// sunqueen modify
                p_data->argc = i_argc;
...
{
    vlc_object_t *p_this = (vlc_object_t *)data;			// sunqueen modify
    HWND ipcwindow;
    MSG message;

    ipcwindow =
        CreateWindow( /*L*/"STATIC",                     /* name of window class */			// sunqueen modify
                  /*L*/"VLC ipc "VERSION,               /* window title bar text */			// sunqueen modify
                  0,                                         /* window style */
...
void system_End(void)
{
    ...
    if (p_helper)
    {
        if( ( ipcwindow = FindWindow( 0, /*L*/"VLC ipc "VERSION ) ) != 0 )			// sunqueen modify
        {
