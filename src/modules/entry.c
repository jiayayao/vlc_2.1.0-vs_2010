...
{
    module_t *module = (module_t *)malloc (sizeof (*module));			// sunqueen modify
    if (module == NULL)
        return NULL;
...
static module_config_t *vlc_config_create (module_t *module, int type)
{
    ...
    if ((confsize & 0xf) == 0)
    {
        tab = (module_config_t *)realloc_or_free (tab, (confsize + 17) * sizeof (*tab));			// sunqueen modify
        if (tab == NULL)
            return NULL;
...
{
    module_t **pprimary = (module_t **)plugin;			// sunqueen modify
    module_t *module = (module_t *)tgt;			// sunqueen modify
    module_config_t *item = (module_config_t *)tgt;			// sunqueen modify
    va_list ap;
    int ret = 0;
    ...
    switch (propid)
    {
        case VLC_MODULE_CREATE:
        {
            ...
            *(va_arg (ap, module_t **)) = submodule;
            ...
            /* Inheritance. Ugly!! */
            submodule->pp_shortcuts = (char **)xmalloc (sizeof (char **));			// sunqueen modify
        ...
        case VLC_MODULE_SHORTCUT:
        {
            ...
            char **pp = (char **)realloc (module->pp_shortcuts,
                                 sizeof (pp[0]) * (index + i_shortcuts));			// sunqueen modify
            if (unlikely(pp == NULL))
            {
                ret = -1;
                break;
        ...
        case VLC_MODULE_NAME:
        {
            ...
            module->pp_shortcuts = (char **)malloc( sizeof( *module->pp_shortcuts ) );			// sunqueen modify
        ...
        case VLC_CONFIG_LIST:
        {
            ...
            if (IsConfigIntegerType (item->i_type))
            {
                ...
                int *dst = (int *)xmalloc (sizeof (int) * len);			// sunqueen modify
            ...
            if (IsConfigStringType (item->i_type))
            {
                ...
                char **dst = (char **)xmalloc (sizeof (char *) * len);			// sunqueen modify
            ...
            char **dtext = (char **)xmalloc (sizeof (char *) * (len + 1));			// sunqueen modify
            for (size_t i = 0; i < len; i++)
