...
module_t **module_list_get (size_t *n)
{
    ...
    {
         module_t **nt;
         nt  = (module_t **)realloc (tab, (i + 1 + mod->submodule_count) * sizeof (*tab));			// sunqueen modify
         if (unlikely(nt == NULL))
         {
             free (tab);
             *n = 0;
...
{
    const module_t *const *ma = (const module_t *const *)a, *const *mb = (const module_t *const *)b;			// sunqueen modify
 ...
 */
ssize_t module_list_cap (module_t ***__restrict list, const char *cap)			// sunqueen modify
{
    ...
    module_t **tab = (module_t **)malloc (sizeof (*tab) * n);			// sunqueen modify
    *list = tab;
