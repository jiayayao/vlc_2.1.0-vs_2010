...
{
    const struct xml_entity_s *ent = (const xml_entity_s *)elem;			// sunqueen modify
    const char *name = (const char *)key;			// sunqueen modify
    ...
    while ( *psz_value )
    {
        if( *psz_value == '&' )
        {
            ...
            {   /* Well-known XML entity */
                ...
                ent = (const xml_entity_s *)bsearch (psz_value + 1, xml_entities,
                               sizeof (xml_entities) / sizeof (*ent),
                               sizeof (*ent), cmp_entity);			// sunqueen modify
                if (ent != NULL)
                {
...
char *convert_xml_special_chars (const char *str)
{
    ...
    char *const buf = (char *)malloc (6 * len + 1), *ptr = buf;			// sunqueen modify
    ...
    *(ptr++) = '\0';
    ...
    ptr = (char *)realloc (buf, ptr - buf);			// sunqueen modify
...
char *vlc_b64_encode_binary( const uint8_t *src, size_t i_src )
{
    ...
    char *ret = (char *)malloc( ( i_src + 4 ) * 4 / 3 );			// sunqueen modify
    char *dst = ret;
...
size_t vlc_b64_decode_binary( uint8_t **pp_dst, const char *psz_src )
{
    ...
    *pp_dst = p_dst = (uint8_t *)malloc( i_src );			// sunqueen modify
    if( !p_dst )
        return 0;
...
{
    const int i_src = strlen( psz_src );
    char *p_dst = (char *)malloc( i_src + 1 );			// sunqueen modify
    size_t i_dst;
    if( !p_dst )
        return NULL;
...
char *str_format_time( const char *tformat )
{
    ...
    {
        char *str = (char *)malloc (buflen);			// sunqueen modify
        if (str == NULL)
            return NULL;
        ...
        if (len > 0)
        {
            char *ret = (char *)realloc (str, len + 1);			// sunqueen modify
...
static void format_duration (char *buf, size_t len, int64_t duration)
{
    ...
    duration /= CLOCK_FREQ;
    d = cover_lldiv (duration, 60);			// sunqueen modify
    sec = d.rem;
    d = cover_lldiv (d.quot, 60);			// sunqueen modify
    _snprintf (buf, len, "%02lld:%02d:%02d", d.quot, (int)d.rem, sec);			// sunqueen modify
}
...
#define INSERT_STRING( string )                                     \
                    ...
                    {                                               \
                        ...
                        dst = (char *)xrealloc( dst, i_size = i_size + len );\
            ...
            switch( *s )
            {
                ...
                case 'f':
                    if( p_item && p_item->p_stats )
                    {
                        ...
                        _snprintf( buf, 10, "%"PRIi64,
                                  p_item->p_stats->i_displayed_pictures );			// sunqueen modify
                ...
                case 'B':
                    if( p_input )
                    {
                        _snprintf( buf, 10, "%"PRId64,
                                  var_GetInteger( p_input, "bit-rate" )/1000 );			// sunqueen modify
                    }
                    else
                        strcpy( buf, b_empty_if_na ? "" : "-" );
                    INSERT_STRING_NO_FREE( buf );
                ...
                case 'C':
                    ...
                    {
                        _snprintf( buf, 10, "%"PRId64,
                                  var_GetInteger( p_input, "chapter" ) );			// sunqueen modify
                    }
                    else
                        strcpy( buf, b_empty_if_na ? "" : "-" );
                    INSERT_STRING_NO_FREE( buf );
                ...
                case 'I':
                    ...
                    {
                        _snprintf( buf, 10, "%"PRId64,
                                  var_GetInteger( p_input, "title" ) );			// sunqueen modify
                    }
                    else
                        strcpy( buf, b_empty_if_na ? "" : "-" );
                    INSERT_STRING_NO_FREE( buf );
                ...
                case 'P':
                    if( p_input )
                    {
                        _snprintf( buf, 10, "%2.1lf",
                                  var_GetFloat( p_input, "position" ) * 100. );			// sunqueen modify
                    }
                    else
                    {
                        _snprintf( buf, 10, b_empty_if_na ? "" : "--.-%%" );			// sunqueen modify
                    }
                    INSERT_STRING_NO_FREE( buf );
                    break;
                case 'R':
                    if( p_input )
                    {
                        ...
                        _snprintf( buf, 10, "%.3f", f );			// sunqueen modify
                    }
                    else
                        strcpy( buf, b_empty_if_na ? "" : "-" );
                    INSERT_STRING_NO_FREE( buf );
                ...
                case 'S':
                    if( p_input )
                    {
                        ...
                        _snprintf( buf, 10, "%d.%d", r/1000, (r/100)%10 );			// sunqueen modify
                    }
                    else
                        strcpy( buf, b_empty_if_na ? "" : "-" );
                    INSERT_STRING_NO_FREE( buf );
                ...
                case 'V':
                {
                    ...
                    {
                        _snprintf( buf, 10, "%ld",
                                  cover_lroundf(vol * AOUT_VOLUME_DEFAULT ) );			// sunqueen modify
                        INSERT_STRING_NO_FREE( buf );
                    }
                    else
