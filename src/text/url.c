...
static char *encode_URI_bytes (const char *str, size_t *__restrict lenp)			// sunqueen modify
{
    char *buf = (char *)malloc (3 * *lenp + 1);			// sunqueen modify
    ...
    *lenp = out - buf;
    out = (char *)realloc (buf, *lenp + 1);			// sunqueen modify
...
# define SMB_SCHEME "file"
        ...
        buf = (char *)malloc (sizeof (SMB_SCHEME) + 3 + hostlen);			// sunqueen modify
        if (buf != NULL)
            _snprintf (buf, sizeof (SMB_SCHEME) + 3 + hostlen,
                      SMB_SCHEME"://%s", path + 2);			// sunqueen modify
        path += 2 + hostlen;
...
char *make_path (const char *url)
{
    ...
    char *path = (char *)strstr (url, "://");			// sunqueen modify
    if (path == NULL)
        return NULL; /* unsupported scheme or invalid syntax */

    end = (char *)memchr (url, '/', path - url);			// sunqueen modify
    ...
    if (schemelen == 4 && !strncasecmp (url, "file", 4))
    {
        ...
        if (*path == '\\')
            return (char *)memmove (path, path + 1, strlen (path + 1) + 1);			// sunqueen modify
        /* Local path disguised as a remote one */
        if (!strncasecmp (path, "localhost\\", 10))
            return (char *)memmove (path, path + 10, strlen (path + 10) + 1);			// sunqueen modify
        /* UNC path */
 ...
 * \return nothing
 */
void vlc_UrlParse (vlc_url_t *__restrict url, const char *str, unsigned char opt)			// sunqueen modify
{
 ...
 */
void vlc_UrlClean (vlc_url_t *__restrict url)			// sunqueen modify
{
    free (url->psz_host);
    free (url->psz_buffer);
}
