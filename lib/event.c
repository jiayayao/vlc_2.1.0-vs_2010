                         ...
                         libvlc_event_listener_t * searched_listener )
{
    ...
    {
        if( listeners_are_equal(searched_listener, (libvlc_event_listener_t *)vlc_array_item_at_index(&group->listeners, i)) )			// sunqueen modify
            return true;
    }
    return false;
}
...
libvlc_event_manager_new( void * p_obj, libvlc_instance_t * p_libvlc_inst )
{
    ...
    p_em = (libvlc_event_manager_t *)malloc(sizeof( libvlc_event_manager_t ));			// sunqueen modify
    if( !p_em )
    {
...
void libvlc_event_manager_release( libvlc_event_manager_t * p_em )
{
    ...
    {
        p_lg = (libvlc_event_listeners_group_t *)vlc_array_item_at_index( &p_em->listeners_groups, i );			// sunqueen modify
        ...
        libvlc_event_type_t event_type )
{
    ...
    listeners_group = (libvlc_event_listeners_group_t *)xmalloc(sizeof(libvlc_event_listeners_group_t));			// sunqueen modify
                        ...
                        libvlc_event_t * p_event )
{
    ...
    for( i = 0; i < vlc_array_count(&p_em->listeners_groups); i++)
    {
        listeners_group = (libvlc_event_listeners_group_t *)vlc_array_item_at_index(&p_em->listeners_groups, i);			// sunqueen modify
        if( listeners_group->event_type == p_event->type )
        {
            ...
            array_listeners_cached = (libvlc_event_listener_t *)malloc(sizeof(libvlc_event_listener_t)*(i_cached_listeners));			// sunqueen modify
            ...
            {
                listener = (libvlc_event_listener_t *)vlc_array_item_at_index(&listeners_group->listeners, i);			// sunqueen modify
...
const char * libvlc_event_type_name( int event_type )
{
    ...
    p = (const event_name_t *)bsearch( &event_type, event_list,
                 sizeof(event_list)/sizeof(event_list[0]), sizeof(*p),
                 evcmp );			// sunqueen modify
                  ...
                  bool is_asynchronous )
{
    ...
    listener = (libvlc_event_listener_t *)malloc(sizeof(libvlc_event_listener_t));			// sunqueen modify
    ...
    {
        listeners_group = (libvlc_event_listeners_group_t *)vlc_array_item_at_index(&p_event_manager->listeners_groups, i);			// sunqueen modify
                                     ...
                                     void *p_user_data )
{
    ...
    for( i = 0; i < vlc_array_count(&p_event_manager->listeners_groups); i++)
    {
        ...
        if( listeners_group->event_type == event_type )
        {
            ...
            {
                listener = (libvlc_event_listener_t *)vlc_array_item_at_index(&listeners_group->listeners, j);			// sunqueen modify
