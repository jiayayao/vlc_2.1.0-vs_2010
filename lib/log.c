...
void libvlc_log_get_context(const libvlc_log_t *ctx,
                            ...
                            unsigned *__restrict line)			// sunqueen modify
{
    if (module != NULL)
...
void libvlc_log_get_object(const libvlc_log_t *ctx,
                           ...
                           const char **__restrict header,
                           uintptr_t *__restrict id)			// sunqueen modify
{
    if (name != NULL)
...
{
    libvlc_instance_t *inst = (libvlc_instance_t *)data;			// sunqueen modify
...
static void libvlc_log_file (void *data, int level, const libvlc_log_t *log,
                             const char *fmt, va_list ap)
{
    FILE *stream = (FILE *)data;			// sunqueen modify
...
{
    (void) p_instance;
    return (libvlc_log_t *)malloc(1);			// sunqueen modify
}
...
{
    return (p_log != NULL) ? (libvlc_log_iterator_t *)malloc(1) : NULL;			// sunqueen modify
}
